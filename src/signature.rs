...
impl<'a> SequoiaMechanism<'a> {
    ...
    fn verify(&mut self, signature: &[u8]) -> Result<SequoiaVerificationResult, anyhow::Error> {
        ...
        match &v.helper_ref().signer {
            ...
            None => Err(anyhow::anyhow!("No valid signer")), // Should not happpen, Helper should have rejected this.
        }
    }
}
...
mod tests {
    ...
    fn import_keys() {
        ...
        // but, purely for test purposes, we can trigger an I/O write error by using a non-ephemeral mechanism
        ...
        let original_perms = fs::metadata(&certstore_dir).unwrap().permissions();
        let mut ro_perms = original_perms.clone();
        ro_perms.set_readonly(true);
        fs::set_permissions(&certstore_dir, ro_perms).unwrap();
