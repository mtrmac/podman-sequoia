...
impl<'a> SequoiaMechanism<'a> {
    ...
    fn sign(
        ...
        let mut sink = vec![];
        {
            ...
            // Coverage: Signer::new() never fails (the underlying code could fail via Signer::with_template
            // if the caller specified an unsupported hash algorithm, but that’s not our case).
...
mod tests {
    ...
    fn sequoia_mechanism_new_from_directory() {
        ...
        // A non-existent absolute path works in sequoia_directories::Home::new,
        // and later fails in Keystore::connect.
        let res =
            SequoiaMechanism::from_directory(Some(Path::new("/var/empty/this/does/not/exist")));
        assert!(res.is_err());
        // Also, test the same thing in C, to exercise the C error reporting code.
        {
            let mut err_ptr: *mut SequoiaError = ptr::null_mut();
            let c_sequoia_home = CString::new("/var/empty/this/does/not/exist").unwrap();
            let m = unsafe {
                super::sequoia_mechanism_new_from_directory(c_sequoia_home.as_ptr(), &mut err_ptr)
            };
            assert!(m.is_null());
    ...
    fn sign() {
        ...
        fn import_key(mech: &mut SequoiaMechanism, cert: &Cert) {
        ...
        let mut mech = SequoiaMechanism::from_directory(Some(fixture_path_buf())).unwrap();
        ...
        // FIXME? We don’t test certs[0].to_cert failing; we’d probably have to use SequoiaMechanism::from_directory
        // against a fixture with invalid certificates.
